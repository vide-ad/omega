...
export function foldConstraints(cs: readonly ExerciseConstraint[]): ConstraintEnvelope {
  const env: ConstraintEnvelope = { blocked: false, requires_clearance: false, max_weight_kg: null, min_reps: null, required_tempo: null, notes: [] };
  for (const c of cs) {
    env.blocked ||= c.blocked;
    env.requires_clearance ||= c.requires_clearance;
